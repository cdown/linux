...
#define printk_store_fmt(func, fmt, ...)				       \
	({								       \
		...
		if (__builtin_constant_p(fmt)) {			       \
			/*						       \
			 * Compiler can't eliminate this branch, so we need to \
			 * make sure it doesn't see an assignment for	       \
			 * non-constants, hence the repeated		       \
			 * __builtin_constant_p(fmt) check.		       \
			 */						       \
...
#define printk(fmt, ...) printk_store_fmt(_printk, fmt,     \
					  ##__VA_ARGS__)
#define printk_deferred(fmt, ...) printk_store_fmt(_printk, \
						   fmt, ##__VA_ARGS__)
