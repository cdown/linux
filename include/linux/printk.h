...
#ifdef CONFIG_PRINTK_ENUMERATION
#define printk_store_fmt(func, var, fmt, ...)				       \
	__builtin_choose_expr(__builtin_constant_p(fmt),		       \
		({							       \
			/*						       \
			 * Compiler can't eliminate this branch, hence the     \
			 * second __builtin_constant_p check		       \
			 */						       \
...
#define printk(fmt, ...) printk_store_fmt(_printk, __UNIQUE_ID(__p_), fmt,     \
					  ##__VA_ARGS__)
#define printk_deferred(fmt, ...) printk_store_fmt(_printk, __UNIQUE_ID(__p_), \
						   fmt, ##__VA_ARGS__)
