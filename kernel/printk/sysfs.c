...
void console_register_device(struct console *con)
{
	...
	/*
	 * Console devices are logical sub-devices that don't have their own
	 * power management. They are managed through their parent device (the
	 * actual hardware console). Mark them as no-callback devices to prevent
	 * runtime PM sysfs attributes from being created and to indicate that
	 * power management is handled by the parent.
	 */
	pm_runtime_no_callbacks(con->classdev);
