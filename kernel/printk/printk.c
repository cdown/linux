...
static size_t ps_get_fmt_output_size(const struct printk_fmt_sec *ps,
				     const char *fmt)
{
	/* eg. vmlinux                          ,   foo: %d\n    \0 */
	return strlen(ps_get_module_name(ps)) + 1 + strlen(fmt) + 1;
}
				 ...
				 const char **end)
{
	...
	for (fptr = ps->start; fptr < ps->end; fptr++)
		size += ps_get_fmt_output_size(ps, *fptr);
...
static void remove_printk_fmt_sec(const struct module *mod)
{
	...
	list_for_each_entry_safe(ps, tmp, &printk_fmts_list, list) {
		if (ps->module == mod) {
			...
			for (fptr = ps->start; fptr < ps->end; fptr++)
				new_size -= ps_get_fmt_output_size(ps, *fptr);
...
static int debugfs_pf_show(struct seq_file *s, void *v)
{
	...
	list_for_each_entry(ps, &printk_fmts_list, list) {
		...
		for (fptr = ps->start; fptr < ps->end; fptr++)
			seq_printf(s, "%s,%s%c", mod_name, *fptr, '\0');
	}
