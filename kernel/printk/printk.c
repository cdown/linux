			    ...
			    size_t count, loff_t *ppos)
{
	...
	if (!printk_get_next_message(&pmsg, NULL, atomic64_read(&user->seq))) {
		...
		ret = wait_event_interruptible(log_wait,
				printk_get_next_message(&pmsg, NULL, atomic64_read(&user->seq))); /* LMM(devkmsg_read:A) */
		if (ret)
			goto out;
	}
 ...
 *
 * The behaviour can be further changed by ignore_per_console_loglevel. If set,
 * existing per-console loglevel values are left intact, but are ignored in
 * favour of console_loglevel as long as this is true. This can also be
 * manipulated through syslog(SYSLOG_ACTION_CONSOLE_{ON,OFF}).
 */
