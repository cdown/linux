...
module_param(ignore_per_console_loglevel, bool, S_IRUGO | S_IWUSR);
 ...
 *    true.
 * 3. con->level. Optional.
 * 4. console_loglevel. The default console loglevel, always present.
 *
 ...
 */
static DEFINE_RATELIMIT_STATE(syslog_percon_ratelimit, 60 * HZ, 1);

static void warn_on_local_loglevel(void)
{
	struct console *con;

	if (!__ratelimit(&syslog_percon_ratelimit))
		return;
...
static int __init console_setup(char *str)
{
	...
	char *s, *options, *res, *brl_options = NULL;
	const char loglevel_prefix[] = ",loglevel:";
	...
	res = strstr(options, loglevel_prefix);
	if (res) {
		*res = '\0';
		res += strlen(loglevel_prefix);
		if (isdigit(res[0])) {
			loglevel = clamp(res[0] - '0', LOGLEVEL_EMERG,
					 LOGLEVEL_DEBUG + 1);
