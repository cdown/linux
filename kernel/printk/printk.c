 ...
 *
 * Per-console loglevels must be positive (> 0). Level 0 (KERN_EMERG) is
 * reserved for emergency messages that should go to all consoles, and -1
 * (LOGLEVEL_DEFAULT) means use the global console_loglevel.
 *
...
/**
 * normalize_console_loglevel - Normalize console loglevel value
 * @con_level: The console loglevel to normalize
 *
 * Normalizes console loglevel values to ensure consistent behavior:
 * - Level 0 (uninitialized or invalid) is converted to LOGLEVEL_DEFAULT
 * - Other values are returned as-is
 *
 * This ensures that level 0, which is explicitly rejected in sysfs and
 * command line parsing, is always treated consistently as "use global
 * loglevel" if it somehow gets set.
 *
 * Return: Normalized loglevel value (LOGLEVEL_DEFAULT for 0, otherwise unchanged)
 */
static int normalize_console_loglevel(int con_level)
{
	/*
	 * Level 0 is not a valid per-console loglevel and should never be
	 * stored. If we encounter it, normalize it to LOGLEVEL_DEFAULT to
	 * ensure consistent behavior (use global loglevel).
	 */
	if (con_level == 0)
...
{
	con_level = normalize_console_loglevel(con_level);
...
int do_syslog(int type, char __user *buf, int len, int source)
{
	...
	case SYSLOG_ACTION_CONSOLE_OFF:
		/*
		 * Note: saved_console_loglevel and saved_ignore_per_console_loglevel
		 * are static variables shared across all processes. This means there
		 * is a potential race condition if multiple processes call CONSOLE_OFF
		 * concurrently. However, this matches the existing legacy behavior for
		 * saved_console_loglevel (which was already static before this change),
		 * so we maintain consistency with the original implementation.
		 */
 ...
 * checks. For devkmsg_read(), CONSOLE_LOGLEVEL_MOTORMOUTH should be used
 * to never suppress messages.
 *
...
void register_console(struct console *newcon)
{
	...
	/* Ensure console->level is initialized if it's uninitialized (0) */
	if (newcon->level == 0 && !(newcon->flags & CON_BRL))
		newcon->level = LOGLEVEL_DEFAULT;
